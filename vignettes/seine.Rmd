...
The only other argument to `ei_spec()` to is `preproc`, which describes preprocessing done to the covariates before model fitting.
...
variables.
Nonparmetric basis expansions like `splines::bs()` and `bases::b_tpsob()` can also be used in the formula interface.
...
```{r}
ei_sens(est_c, c_outcome = 0.5, c_predictor = 0.2)
```
...
```{r}
ei_sens(est_c, c_outcome = 1, bias_bound = 0.05)
```
...
```{r include=FALSE}
fmt_pp = \(x) scales::percent(x, suffix="pp")
est_pt = subset(est_c, outcome=="pres_rep_nix")$estimate 
bb = subset(bench, covariate == "state" & outcome == "pres_rep_nix")
est_bias = subset(ei_sens(est_c, bb$c_outcome, bb$c_predictor), outcome == "pres_rep_nix")$bias_bound
...
sens = ei_sens(est_c) # the default evaluates on a grid of parameters
...
strength could lead to bias of around `r fmt_pp(est_bias)`, which is substantial compared to the
estimate itself, which is `r fmt_pp(est_pt)`.
...
a given amount.
Here, we might consider be interested in bias that would be sufficient to
eliminate any evidence of racially polarized voting, i.e., bias equal to the esimated difference between White and Black voters.

```{r}
ei_sens_rv(est_c, bias_bound = estimate)
```

The robustness value (one for each predictor/outcome combination) is
relatively small for Wallace's vote share, indicating low robustness (high sensitivity).
In particular, it is far smaller than the amount of confounding benchmarked by the observed `state` variable.
For Humphrey and Nixon's vote shares, however, the robustness values are larger, indicating more confidence in the finding of racially polarized voting for those candidates.
